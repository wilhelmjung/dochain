...
注意事项：
- `--excel` 仅支持 `--engine smart` 或 `--engine baidu`
- `smart` 回退到通用 OCR（`general`）时，只返回纯文本，不会写入 Excel 行
- 混合目录批量导出时，非发票/非火车票文件会被跳过，不会生成“未知”空白记录
...
`test_excel_export.sh` 会先验证不支持的引擎（如 `api`）在 `--excel` 模式下被正确拒绝，再执行一次 `smart -> Excel` 的端到端导出和内容校验。
